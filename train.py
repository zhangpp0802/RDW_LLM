...
import time
from generate_agent_points.generate import process_image
...
def train(epoch, model, reg_criterion, cls_criterion, optimizer, train_dataloader, motion_modes):
    ...
    for i, (ped, neis, mask) in enumerate(train_dataloader):
        # ped = ped.cuda()
        neis = neis.cuda()
        # print(neis.shape) #torch.Size([281, 1, 20, 2]) torch.Size([512, 2, 20, 2])
        mask = mask.cuda() 
        ...
        ped = ped.cuda() #torch.Size([512, 20, 2])
        num_included = ped.shape[1]
        ped_path = "/depot/bera89/data/zhan5058/TUTR/pedestrain_process/00%d.pkl"%i
        with open(ped_path, "rb") as f:
            object = pkl.load(f)
        df = pd.DataFrame(object)
        p1 = df["p1"] 
        ped_temp = p1[0]['head']['pos'][:,:2]#(641, 2)
        # print(ped_temp.shape)
        # zeros_array = np.zeros_like(ped_temp)
        # ped_temp = np.stack((ped_temp, zeros_array), axis=0)
        # print(ped_temp)

        ped_temp = ped_temp[:num_included].astype(np.float32)
        # ped_temp = np.expand_dims(ped_temp, axis=0)
        arr_repeated_ped = np.repeat(ped_temp[np.newaxis, ...], ped.shape[0], axis=0)
        # print(arr_repeated.shape)
        # exit()
        ped = torch.tensor(arr_repeated_ped).cuda()

        neis = neis.cuda()
        # print(neis.shape) #torch.Size([512, 2, 20, 2])
        # print(neis[0])
        num_included = neis.shape[2]
        # print(num_included)
        image_path = "/depot/bera89/data/zhan5058/TUTR/generate_agent_points/00%d.png"%i
        ...
        neis_temp=np.array(boundaries).reshape(-1, 2)[:num_included].astype(np.float32)
        zeros_array = np.zeros_like(neis_temp)
        neis_temp = np.stack((neis_temp, zeros_array), axis=0)
        arr_repeated_nei = np.repeat(neis_temp[np.newaxis, ...], neis.shape[0], axis=0)
        neis = torch.tensor(arr_repeated_nei).cuda()
        ...
        optimizer.zero_grad()
        # print(closest_mode_indices.shape) torch.Size([512])
        # print(ped_obs.shape)
        # print(neis_obs.shape)
        # print(mask.shape)
        # exit()
...
def train_a(epoch, model, reg_criterion, cls_criterion, optimizer, train_dataloader, motion_modes):
    model.train()
    total_loss = []
    for i, (ped, neis, mask) in enumerate(train_dataloader):
        ped = ped.cuda()
...
def test_a(model, test_dataloader, motion_modes):
...
def test(model, test_dataloader, motion_modes):
    ...
    for (ped, neis, mask) in test_dataloader:
        ...
        ped = ped.cuda() #torch.Size([512, 20, 2])
        num_included = ped.shape[1]
        ped_path = "/depot/bera89/data/zhan5058/TUTR/pedestrain_process/006.pkl"
        with open(ped_path, "rb") as f:
            object = pkl.load(f)
        df = pd.DataFrame(object)
        p1 = df["p1"] 
        ...
        neis = neis.cuda()
        # print(neis.shape) #torch.Size([512, 1, 20, 2])
        num_included = neis.shape[2]
        # print(num_included)
        ...
        # print(np.array(boundaries).shape)
        neis_temp=np.array(boundaries).reshape(-1, 2)[:num_included].astype(np.float32)
        neis_temp = np.expand_dims(neis_temp, axis=0)
        arr_repeated_nei = np.repeat(neis_temp[np.newaxis, ...], neis.shape[0], axis=0)
        neis = torch.tensor(arr_repeated_nei).cuda()
        # print(neis.float().shape)
        # neis = neis.to(torch.float32)#torch.Size([512, 1, 20, 2])
        # print(neis.shape)
        # exit()
        mask = mask.cuda() 
        ped_obs = ped[:, :args.obs_len] #torch.Size([512, 8, 2])
        gt = ped[:, args.obs_len:] #torch.Size([512, 12, 2])
        # print(ped_obs.shape)
        # print(gt.shape)
        # exit()
        neis_obs = neis[:, :, :args.obs_len]
